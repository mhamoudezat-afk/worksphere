...
export default function StatsCard({ title, value, prefix = '', suffix = '', change = 0, icon: Icon, color }: StatsCardProps) {
  ...
  return (
    ...
    >
      ...
      />
      <motion.div 
        className="absolute -bottom-20 -left-20 w-40 h-40 bg-gradient-to-tr from-white/5 to-transparent rounded-full blur-2xl"
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-3">
          ...
          {change !== 0 && (
            <motion.div 
              ...
              className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${isPositive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}
            >
        ...
        <div className="flex items-baseline gap-1">
          ...
          <span className={`text-2xl font-bold tracking-tight ${textColors[color]}`}>
        ...
        {color === 'purple' && (
          <div className="mt-3">
            <div className="h-1 bg-gray-700 rounded-full overflow-hidden">
              <motion.div 
                initial={{ width: 0 }}
                animate={{ width: `${Math.min((value / 100000) * 100, 100)}%` }}
